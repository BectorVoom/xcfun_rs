"""mpmath port of xcfun-master/src/functionals/r2SCANx.cpp at mp.prec=200.

r2SCAN exchange. Plan 06-N2 Task 1b. Settings: IALPHA=2, IINTERP=1, IDELFX=1.
"""
...
def _value_r2scanx(*inputs, vars_str):
...
def eval_r2scanx(inputs, vars, mode, order):
    mp.mp.prec = 200
    pt = tuple(mp.mpf(x) for x in inputs)
    return multivariate_taylor(
        lambda *args, _vs=vars: _value_r2scanx(*args, vars_str=_vs),
        pt,
        order,
    )

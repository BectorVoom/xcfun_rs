"""mpmath port of xcfun-master/src/functionals/rSCANc.cpp at mp.prec=200.

rSCAN correlation. Plan 06-N2 Task 1b. Settings: IALPHA=1, IINTERP=1, IDELEC=0.
"""
...
def _value_rscanc(*inputs, vars_str):
...
def eval_rscanc(inputs, vars, mode, order):
    mp.mp.prec = 200
    pt = tuple(mp.mpf(x) for x in inputs)
    return multivariate_taylor(
        lambda *args, _vs=vars: _value_rscanc(*args, vars_str=_vs),
        pt,
        order,
    )

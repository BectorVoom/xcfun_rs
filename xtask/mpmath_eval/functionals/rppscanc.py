"""mpmath port of xcfun-master/src/functionals/rppSCANc.cpp at mp.prec=200.

r++SCAN correlation. Plan 06-N2 Task 1b. Settings: IALPHA=2, IINTERP=1, IDELEC=0.
"""
...
def _value_rppscanc(*inputs, vars_str):
...
def eval_rppscanc(inputs, vars, mode, order):
    mp.mp.prec = 200
    pt = tuple(mp.mpf(x) for x in inputs)
    return multivariate_taylor(
        lambda *args, _vs=vars: _value_rppscanc(*args, vars_str=_vs),
        pt,
        order,
    )

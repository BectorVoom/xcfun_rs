"""mpmath port of xcfun-master/src/functionals/brx.cpp::brxc at mp.prec=200.

Becke-Roussel exchange-correlation composite. Plan 06-N2 Task 1a
(W-6 revision-2 — BR family).

The BRXC body (`brx.cpp:123-136`) is BR-correlation plus the BR-exchange
contribution `0.5 * (UXa * a + UXb * b)`. Constants cab=0.63, caa=0.88
matched verbatim. mpmath at prec=200 is sole ground truth.
"""
...
def _value_brxc(*inputs, vars_str):
    ...
    return mp.mpf("0.5") * (UXa * a + UXb * b) + ECopp + ECaa + ECbb


def eval_brxc(inputs, vars, mode, order):
    mp.mp.prec = 200
    pt = tuple(mp.mpf(x) for x in inputs)
    return multivariate_taylor(
        lambda *args, _vs=vars: _value_brxc(*args, vars_str=_vs),
        pt,
        order,
    )

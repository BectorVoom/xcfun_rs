"""mpmath port of xcfun-master/src/functionals/r4SCANc.cpp at mp.prec=200.

r4SCAN correlation. Plan 06-N2 Task 1b. Settings: IALPHA=2, IINTERP=1, IDELEC=2.
"""
...
def _value_r4scanc(*inputs, vars_str):
...
def eval_r4scanc(inputs, vars, mode, order):
    mp.mp.prec = 200
    pt = tuple(mp.mpf(x) for x in inputs)
    return multivariate_taylor(
        lambda *args, _vs=vars: _value_r4scanc(*args, vars_str=_vs),
        pt,
        order,
    )

"""mpmath port of xcfun-master/src/functionals/rSCANx.cpp at mp.prec=200.

rSCAN exchange. Plan 06-N2 Task 1b. Settings: IALPHA=1, IINTERP=1, IDELFX=0.
"""
...
def _value_rscanx(*inputs, vars_str):
...
def eval_rscanx(inputs, vars, mode, order):
    mp.mp.prec = 200
    pt = tuple(mp.mpf(x) for x in inputs)
    return multivariate_taylor(
        lambda *args, _vs=vars: _value_rscanx(*args, vars_str=_vs),
        pt,
        order,
    )

//! XC_R2SCANX — r2SCAN exchange functional. MGGA-02.
//!
//! Port of `xcfun-master/src/functionals/r2SCANx.cpp`.
//! Uses `get_SCAN_Fx(2*d.a, 4*d.gaa, 2*d.taua, IALPHA=2, IINTERP=1, IDELFX=1)`.
...
#[cube]
fn r2scan_exchange_spin<F: Float>(
    rho: &Array<F>,
    grad2: &Array<F>,
    tau: &Array<F>,
    out: &mut Array<F>,
    ...
    scan_like::get_SCAN_Fx::<F>(&two_rho, &four_grad2, &two_tau, &mut fx, 2_u32, 1_u32, 1_u32, n);
...
pub fn r2scanx_kernel<F: Float>(d: &DensVarsDev<F>, out: &mut Array<F>, #[comptime] n: u32) {
    let size = comptime!((1_u32 << n) as usize);

    let mut ea = Array::<F>::new(size);
    r2scan_exchange_spin::<F>(&d.a, &d.gaa, &d.taua, &mut ea, n);

    let mut eb = Array::<F>::new(size);
    r2scan_exchange_spin::<F>(&d.b, &d.gbb, &d.taub, &mut eb, n);

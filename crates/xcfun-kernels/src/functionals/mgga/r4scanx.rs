//! XC_R4SCANX — r4SCAN exchange functional. MGGA-02.
//!
//! Port of `xcfun-master/src/functionals/r4SCANx.cpp`.
//! Uses `get_SCAN_Fx(2*d.a, 4*d.gaa, 2*d.taua, IALPHA=2, IINTERP=1, IDELFX=2)`.
//! IDELFX=2 triggers the 4th-order gradient expansion (HIGH-RISK per CONTEXT D-11).
//! Vars: XC_A_B_GAA_GAB_GBB_TAUA_TAUB (id=13, inlen=7).
//!
//! # R4SCAN accuracy note (CONTEXT D-11 watch list)
//! The 4th-order polynomial in `SCAN_X_Fx` (IDELFX=2 branch) uses explicit-let
//! bindings throughout to prevent any compiler reassociation. No `mul_add` used.
...
#[cube]
fn r4scan_exchange_spin<F: Float>(
    rho: &Array<F>,
    grad2: &Array<F>,
    tau: &Array<F>,
    out: &mut Array<F>,
...
pub fn r4scanx_kernel<F: Float>(d: &DensVarsDev<F>, out: &mut Array<F>, #[comptime] n: u32) {
    let size = comptime!((1_u32 << n) as usize);

    let mut ea = Array::<F>::new(size);
    r4scan_exchange_spin::<F>(&d.a, &d.gaa, &d.taua, &mut ea, n);

    let mut eb = Array::<F>::new(size);
    r4scan_exchange_spin::<F>(&d.b, &d.gbb, &d.taub, &mut eb, n);

//! XC_RPPSCANX — r++SCAN exchange functional. MGGA-02.
//!
//! Port of `xcfun-master/src/functionals/rppSCANx.cpp`.
//! Uses `get_SCAN_Fx(2*d.a, 4*d.gaa, 2*d.taua, IALPHA=2, IINTERP=1, IDELFX=0)`.
...
#[cube]
fn rppscan_exchange_spin<F: Float>(
    rho: &Array<F>,
    grad2: &Array<F>,
    tau: &Array<F>,
    out: &mut Array<F>,
...
pub fn rppscanx_kernel<F: Float>(d: &DensVarsDev<F>, out: &mut Array<F>, #[comptime] n: u32) {
    let size = comptime!((1_u32 << n) as usize);

    let mut ea = Array::<F>::new(size);
    rppscan_exchange_spin::<F>(&d.a, &d.gaa, &d.taua, &mut ea, n);

    let mut eb = Array::<F>::new(size);
    rppscan_exchange_spin::<F>(&d.b, &d.gbb, &d.taub, &mut eb, n);

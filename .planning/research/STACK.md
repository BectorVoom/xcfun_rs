# Stack Research — xcfun_rs

**Domain:** Rust-native numerical library reimplementing a C++ scientific (DFT exchange-correlation) engine with ≤ 1e-12 output parity, plus C-ABI, Python, and unified CPU/GPU execution.
**Researched:** 2026-04-18 (ecosystem re-checked vs. crates.io live registry).
**Overall confidence:** HIGH for language/error/FFI picks; MEDIUM for GPU pick; the GPU selection is the only one where outside risk meaningfully remains.
...
Nothing in the existing stack is wrong; three deps are stale (`bitflags`, `cc`, `proptest`/`rstest`/`rand_xoshiro` cluster), and **cubecl** is the one load-bearing pre-release dependency requiring continuous validation-harness gating.
...
**Rejected AD crates (unchanged from prior research, re-verified on crates.io 2026-04):**
...
**Why not alternatives** (verified against crates.io 2026-04):
...
**C-ABI crate shape:** `xcfun-capi` exports both `cdylib` (for drop-in replacement of `libxcfun.so`) and `staticlib` (for consumers that statically link). `crate-type = ["cdylib", "staticlib"]` is correct in the current design. No change.
...
**Rejected:** `cpython` (unmaintained, PyO3 superseded it), `rustpython` (embedded interpreter, not a binding), `pyrust` (stale).
...
Workspace `Cargo.toml` `[workspace.dependencies]` block with the proposed version moves applied:
...
thiserror   = "=2.0.18"
bitflags    = "=2.11.1"      # was 2.10.0 — moved
...
cbindgen    = "=0.29.2"
cc          = { version = "^1.2.60", features = ["parallel"] }  # was "1.1"
criterion   = { version = "=0.8.2", default-features = false, features = ["html_reports"] }
approx      = "=0.5.1"        # hold at 0.5 — 0.6.0-rcN not stable yet
proptest    = "=1.11.0"       # was 1.6
rstest      = "=0.26.1"       # was 0.23
anyhow      = "^1.0.102"      # was "1.0"
serde_json  = "^1.0.149"
rand_xoshiro = "=0.8.0"       # was 0.7
tracing-subscriber = "=0.3.23"  # was 0.3 unbounded
```

Build/development tools (not Cargo dependencies):
...
`RUSTFLAGS` in CI: **empty**. No `-Cfast-math`, no `-Ctarget-cpu=native`, no reassociation flags. This is an accuracy-contract invariant (see `07-accuracy-strategy.md §2`).
...
**If the caller wants CPU-only, CUDA-free builds:**
...
**If the caller wants maximum GPU throughput (HPC workload):**
...
**If the caller wants macOS/Metal or browser/WebGPU portability:**
...
**If the caller wants Python-only, drop-in pyxcfun replacement:**
...
**If the caller wants a C drop-in for `libxcfun.so`:**
...
Caret (`^`) is fine for `anyhow`, `serde_json`, and `cc` — they are strict app-boundary tools, not inputs to the accuracy contract.
...
**Overall:** Project stack is close to optimal. Apply the six proposed version bumps (`bitflags`, `cc`, `proptest`, `rstest`, `rand_xoshiro`, `tracing-subscriber`) and keep the `=0.10.0-pre.3` cubecl pin under active CI scrutiny. No re-architecture implied.

---

*Stack research for: Rust numerical scientific library (DFT XC engine) with C-ABI + Python + unified CPU/GPU execution.*

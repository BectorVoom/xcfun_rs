...
### Added 2026-04-20 (research escalation, user-approved via plan-phase)

- **D-24:** **LDAERF tier-2 override at 1e-7 (sourced from upstream).** `XC_LDAERFX`, `XC_LDAERFC`, `XC_LDAERFC_JT` use a per-functional tier-2 rel-error threshold of **1e-7**, matching upstream xcfun's own `test_threshold` at `xcfun-master/src/functionals/ldaerfx.cpp:66`. Root cause: cubecl 0.10-pre.3 `Float::erf` polyfill (~1.3e-8 ULP) propagates to ~2e-8 final-output rel-error vs C++ libm `erf` in the LDAERF chain (research §"D-19 LDAERF Tolerance Analysis"). The remaining 8 LDA functionals (`XC_SLATERX, XC_VWN3C, XC_VWN5C, XC_PW92C, XC_PZ81C, XC_TFK, XC_TW, XC_VWK`) hold strict 1e-12 against C++. The override is documented in `report.html` as a per-functional divergence note (NOT silent widening). Phase 6 revisits with libm-hybrid when CUDA/Wgpu `erf` drift also enters scope. **Authorisation:** Per-functional override per CONTEXT D-19, escalated 2026-04-20, approved by user via `/gsd-plan-phase 2` interactive prompt.
- **D-25:** **`XcError::UnknownName` carries no payload.** To satisfy CORE-04's `Copy` requirement, `XcError::UnknownName` is defined without an inner `String` (the original pre-pivot scaffold's `UnknownName(String)` is dropped). Error message at the `Display` impl: `"unknown functional name"` — the caller already has the name they passed; losing it from the error is cosmetic. `XcError` stays `Copy + Clone + Debug + #[non_exhaustive] + thiserror::Error` with 9 variants. **Authorisation:** Resolved 2026-04-20 via `/gsd-plan-phase 2` interactive prompt; user-approved drop-payload option.
